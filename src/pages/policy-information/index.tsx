...
import PolicyFilterForm from "features/master/forms/policy-filter";
import {
  PolicyInfoFormDialog,
  PolicyInfoFormDialogRef,
} from "features/master/forms/policy-information";
...
const PolicyInfoPage: NextPageWithLayout = () => {
  const dispatch = useDispatch();
  const allPolicyInfo = useSelector(selectAllPolicyInfo);
  const policyInfoFormDialogRef = useRef<PolicyInfoFormDialogRef>(null);
  const translate = useTranslate();
  const { user } = useAuth();
      ...
      <Stack direction="row" justifyContent="space-between" sx={{ my: 2 }}>
        ...
        <Box>
          {hasCreatePermissions && ( // Only render the edit icon if the user has edit permissions
            <Button
              variant="contained"
              onClick={() => policyInfoFormDialogRef.current?.open()}
            >
              {translate("policy", "btn_text")}
            </Button>
          )}

          <PolicyInfoFormDialog ref={policyInfoFormDialogRef} />
        </Box>
      </Stack>
...
const PolicyStatusLabel = (policy: PolicyInfo) => {
  ...
  // return (
  //   <>
  //     <Tooltip title={translate("policy", "status_tooltip_text")}>
  //       <Chip
  //         size="small"
...
const TableActions = ({
  ...
  policyInfoFormDialogRef: MutableRefObject<PolicyInfoFormDialogRef>;
}) => {
  const dispatch = useDispatch();
  ...
  const onEditClick = useCallback(() => {
    policyInfoFormDialogRef.current?.open({
      id: policyInfo.id,
...
PolicyInfoPage.getLayout = (page) => {
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const translate = useTranslate();
  return (
    <DashboardLayout>
      <Page title={translate("policy", "title")}>{page}</Page>
    </DashboardLayout>
  );
};
